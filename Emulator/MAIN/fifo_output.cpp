...
float fifo_output_data_wr[1000];
...
void output_fifo::t_source1(void) {
  ...
  if (packet_fifo.nb_write(fifo_output_data_wr[val])) {
    std::cout << sc_time_stamp() << ": output fifo t_thread1(): Wrote " << fifo_output_data_wr[val] << std::endl;
    val++;
  } else {
