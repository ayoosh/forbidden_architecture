...
for $p(0..($cur_input-1)){
	...
	$present_schedule[$layer_sch_count+5] = $present_schedule[$layer_sch_count+5] | $buf{"set_sigmoid_fifo_write_enable"};
...
$present_schedule[$layer_sch_count+3] = $present_schedule[$layer_sch_count+3] & $buf{"clear_sigmoid_unit_input_enable"} & $buf{"clear_pe_write_enable"} & $buf{"clear_input_fifo_enable"};
$present_schedule[$layer_sch_count+4] = $present_schedule[$layer_sch_count+4] & $buf{"clear_sigmoid_unit_input_enable"} & $buf{"clear_pe_write_enable"} & $buf{"clear_input_fifo_enable"};

$layer_sch_count = $layer_sch_count + 4;
...
for $p(0..($cur_input-1)){
        ...
        if($p==0){
                $present_schedule[$layer_sch_count+3] = $present_schedule[$layer_sch_count+2];
                $present_schedule[$layer_sch_count+4] = $present_schedule[$layer_sch_count+2]; }
        $present_schedule[$layer_sch_count+5] = $present_schedule[$layer_sch_count+5] | $buf{"set_output_fifo_write_enable"};
        $layer_sch_count = $layer_sch_count + 1;
}
...
#	print "Layer count for now is $layer_count \n" ;
	...
	for $k($layer_sch_count..($layer_sch_count+$neuron_cur-1)){
		...
		#if($layer_count == 1){  #when layer count is 1, the output goes to output fifo
                #	$present_schedule[$layer_sch_count+3] = $present_schedule[$layer_sch_count] | $buf{"set_output_fifo_write_enable"} ;
		#}
		#else { #when layer count is other than 1, output goes to sigmoid fifo
			$present_schedule[$layer_sch_count+3] = $present_schedule[$layer_sch_count] | $buf{"set_sigmoid_fifo_write_enable"} ;
		#}
		$layer_sch_count = $layer_sch_count + 1 ;
